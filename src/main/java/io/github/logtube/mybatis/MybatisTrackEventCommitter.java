...
public class MybatisTrackEventCommitter {
    ...
    @NotNull
    @Contract("_ -> this")
    public MybatisTrackEventCommitter setInvocation(@NotNull Invocation invocation) {
        this.event.extra("method", invocation.getMethod().getName());
        ...
        this.event.extra("sql", boundSql.getSql()); // this automatically set sqlDigest
        this.event.extra("database_url", extractDatabaseURL(executor));
        try {
            this.event.extra("raw_sql", compileSQL(mappedStatement.getConfiguration(), boundSql));
        } catch (Exception e) {
            this.event.extra("raw_sql", "Failed to compile:" + e.getMessage());
        }
        LOGGER.debug("reflection duration = {} ms", System.currentTimeMillis() - this.startTime);
        return this;
    }

    @NotNull
    @Contract("_ -> this")
    public MybatisTrackEventCommitter setThrowable(@Nullable Throwable e) {
        if (e == null) {
