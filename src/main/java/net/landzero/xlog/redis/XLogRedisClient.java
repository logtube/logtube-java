 ...
 */
public class XLogRedisClient extends Client {
    TrackEventBuilder builder;

    private static final Logger LOGGER = LoggerFactory.getLogger(XLogRedisClient.class);
    ...
    public XLogRedisClient() {
        super();
    }

    public XLogRedisClient(final String host) {
        super(host);
    }

    public XLogRedisClient(final String host, final int port) {
        super(host, port);
    }

    public XLogRedisClient(final String host, final int port, final boolean ssl) {
        super(host, port, ssl);
    }
    ...
    private void sendComandBefore(Command cmd, byte[]... args) {
        needSkipLog = LOG_BLACK_LIST.contains(cmd);
        if (needSkipLog) {
            return;
        }
        try {
            builder = new TrackEventBuilder();
    ...
    private void finished(Object res) {
        ...
        try {
            XLog.appendEvent(builder.build(res));
        } catch (Throwable e) {
            // 预防出错影响正常流程
            LOGGER.error(e.getMessage(), e);
        }
